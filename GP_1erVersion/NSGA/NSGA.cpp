...
NSGA::NSGA(MOP *mop, Randomizer *r, string outputDir, NSGAParams &gaParams)
{
   this->mop = mop;
   this->r = r;
...
//   initialize_pop(mixed_pop);
   ...
   for (int p = 0; p < popsize; ++p) {
 	  if (parent_pop->ind[p]->constr_violation > minDist)
 		  minDist = parent_pop->ind[p]->constr_violation;
	  }
   cout << "\n\t\t\t\t\tDistancia mínima= " << minDist << endl;

   cout << "\n <<<< Fin generación = 1\n\n" << endl;

   //int printingRate = 1 + ngen/5;
   int printingRate = 1;
...
//   archivePopulation(parent_pop);
   ...
   for (i=2; i <= ngen; ++i)
   {
      ...
      cout << "\n\t\t\t\t\tDistancia mínima global= " << minDist << endl;
      if ((i % printingRate) == 0)
    	  cout << "\n\n <<<< Fin  generación = " << i << "\n\n\n" << endl;
   }
...
void NSGA::openInfoFiles() {
   file_firstPop.open(outputDir + name_firstPop, ios::out);       //fpt1
   file_everyPop.open(outputDir + name_everyPop, ios::out);       //fpt4
   file_finalPop.open(outputDir + name_finalPop, ios::out);       //fpt2
   file_feasiblePop.open(outputDir + name_feasiblePop, ios::out); //fpt3
   file_params.open(outputDir + name_params, ios::out);           //ftp5

   file_varSpace.open(outputDir + name_varSpace, ios::out);
   file_objSpace.open(outputDir + name_objSpace, ios::out);
...
void NSGA::report_pop_objs(Population *pop, ostream &output) const
{
   ...
   for (i=0; i < pop->getSize(); i++)
   {
       ...
       output << "\t\t" << setw(5) << pop->ind[i]->constr_violation;
